...
SOURCE_CODE_TYPE_COLUMN = "source_code_type"
...
def _validate_header(headers: List[str], csv_file: Path) -> tuple[List[int], bool]:
...
fullname, domain_id, source_code, highlights, transformation_display_name\""""
    ...
    # check whether optional source_code_type column is present
    has_source_code_type = headers[-1] == SOURCE_CODE_TYPE_COLUMN

    expected_tail = MANDATORY_TAIL + ([SOURCE_CODE_TYPE_COLUMN] if has_source_code_type else [])

    if headers[index_fullname[0]: index_fullname[0] + 2] + headers[index_fullname[1]:] != expected_tail:
        raise InvalidCSVException(
            f"""The header of the csv file {csv_file} does not match the required input;
not all mandatory columns are provided. Example of valid header:
\"Database, Schema, Table, Column, fullname, domain_id, Database, Schema, Table, Column,
fullname, domain_id, source_code, highlights, transformation_display_name\"
Optionally, a \"source_code_type\" column can be added at the end with values 'file' or 'query'."""
        )

    return index_fullname, has_source_code_type
...
def _create_source_code(
    ...
    if not source_code_text or not source_code_text.strip():
        if source_code_type == "file":
            logging.warning(
                f"source_code_type is set to 'file' but source_code is empty (line {line}) — "
                "skipping source code file generation."
            )
        return None

    if source_code_type and source_code_type not in ("file", "query"):
        logging.warning(
            f"Unrecognized source_code_type value '{source_code_type}' (line {line}). "
            f"Falling back to auto-detection. Valid values are 'file' or 'query'."
        )
...
Expected format: \"[0:100]\" (single) or \"[0:100],[200:100]\" (multiple)"""
    ...
    return generate_source_code(
        ...
        source_code_type=source_code_type if source_code_type else None,
...
def ingest_csv_files(source_directory: str, custom_lineage_config: CustomLineageConfig) -> None:
    ...
    for csv_file_to_ingest in csv_files:
        with open(csv_file_to_ingest, "r", encoding="utf-8-sig") as csv_file:
            ...
            index_fullname_src, index_fullname_trg = _validate_header(
                headers=headers, csv_file=csv_file_to_ingest
            )[0], _validate_header(headers=headers, csv_file=csv_file_to_ingest)[0]

            result = _validate_header(headers=headers, csv_file=csv_file_to_ingest)
            index_fullname = result[0]
            has_source_code_type = result[1]
            index_fullname_src = index_fullname[0]
            index_fullname_trg = index_fullname[1]
...
of entries as the header"""
                ...
                tail = row[index_fullname_trg + 2:]
                source_code_text = tail[0]
                highlights = tail[1]
                transformation_display_name = tail[2]
                source_code_type = tail[3] if has_source_code_type and len(tail) > 3 else None
